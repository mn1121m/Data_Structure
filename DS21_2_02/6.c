...
04. 배열, 구조체, 다항식더하기
6. [2차원 배열을 함수로 전달] 정적배열, 동적배열을 함수로 전달하는 방법을 비교하는 프로그램이다.
함수의 파라미터 타입에 주의하면서 프로그램을 작성하라.
*/
#include <stdio.h>
#include <stdlib.h>
...
sumAry2D_f5(): 9
-------------------------------------------------------------
+ 동적배열을 sumAry2D_f1의 인자로 전달할 수 없는 이유
    동적배열 -> 정적배열 : 크기가 다르기 때문입니다. 
    int ** 의 크기는 (sizeof) = 4, int [*][3]의 크기는 4 * 3 = 12이므로 4 -> 12로 전달할 수 없습니다.

+ 정적배열을 동적배열로 전달할 수 없는 이유 
    정적배열인 int [2][3] 의 크기는 4 * 2 * 3 = 24, int ** 의 크기는 4입니다.
    각각의 type형이 다르기 때문입니다. 
*/
